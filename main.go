...
var (
	...
	basePath    = env.String("BASE_PATH", false, "./repos", "Base path to save repositories")
)
...
func main() {
	...
	ph.HandleFunc("/images/{filename:[a-zA-Z]+\\.[a-z]{3}}", fh.UploadREST)

	// get files
	gh := sm.Methods(http.MethodGet).Subrouter()
	gh.Handle(
		"/api/v1/uploads/{id:[0-9a-f-]{36}}",
		http.StripPrefix("/api/v1/uploads/", http.FileServer(http.Dir(*basePath))),
	)
